...
export default function DestinationLayout() {
  ...
  return (
    <div>
      ...
      <img src={tabletDestinationBg} alt="background-destination-tablet" className=" hidden md:block md:h-screen relative" />

      <div className=" absolute top-28 w-full px-6 md:px-10 md:top-[136px]">
        <div className=" flex flex-col items-center">

          <p className=" font-barlow-Condensed text-base text-white uppercase tracking-[2.7px] mb-8 md:w-full md:text-xl md:mb-16">
          ...
          {data.destinations.map((destination, index) => {
            if (index === id) {
              return (
                <div key={index}>
                  <img src={destination.images.png} alt="" className=" w-[170px] h-[170px] md:w-[300px] md:h-[300px]" />
                </div>
              );
            }
          })}
          ...
          {data.destinations.map((destination, index) => {
            if (index === id) {
              return (
                <p className=" text-[3.5rem] text-white font-bellefair uppercase leading-[1.15] tracking-[4px] mb-7 md:text-[5rem] md:mb-8" key={index}>{destination.name}</p>
              );
            }
          })}
          ...
          {data.destinations.map((destination, index) => {
            if (index === id) {
              return (
                <p className=" text-center text-base tracking-widest leading-[25px] mb-8 md:leading-7 md:mx-24 md:mb-14" key={index}>{destination.description}</p>
              );
            }
          })}

          {/* horizontal line */}
          <div className=" h-[1px] w-full bg-[#383B4B] md:w-[573px] md:mb-7"></div>
          ...
          <div className=" md:flex md:gap-[100px]">
            ...
            {data.destinations.map((destination, index) => {
              if (index === id) {
                return (
                  <div className=" flex flex-col items-center my-8 md:my-0" key={index}>
